...
|------|-----------|---------|
| MongoDB ObjectID (24-char hex) | `customers`, `invoices`, `journals`, `vehicles`, `route-assignments`, `pickups/route` | `"674a1b2c3d4e5f6789012345"` |
...
## Active routes — permission: `active_route`

One active route per pickup or delivery (+ container) per calendar day. References a dispatch route from `/routes`.

### `POST /v1/active-routes`

`name` is generated by the API on create — do not send it.

```json
{
  "container": null,
  "date": "2026-06-10T00:00:00Z",
  "route": { "id": "674a1b2c3d4e5f6789012345" },
  "driver": { "id": 5, "name": "Jane Driver" },
  "appraiser": { "id": 6, "name": "Helper One" }
}
```

For a **pickup** active route, send `container: null`. For a **delivery** active route, send `container: { "id": 25, "name": "CONT-25" }`. `route.id` is the Mongo ObjectId from `/routes`. `driver` and `appraiser` are optional employee refs. Response includes server-generated `name` (pickup: `date-employee1-employee2-…-vehicle`; delivery: `01-containerNumber-year`).

### `PUT /v1/active-routes/{id}`

Same write shape (omit `name` unless explicitly updating). `{id}` = ObjectID hex.

Lookup by type + date: `POST /v1/active-routes/search` with filters on `date` and `container` (`null` for pickup, or `container.id` for delivery).
...
`name` and `routeId` are generated by the API on create — do not send them. Date and container belong on **active routes**, not routes.
...
Response includes server-generated `name` (`employee1-employee2-…-vehicle` naming convention) and `routeId`. Date and container belong on **active routes**, not routes.
...
**`<resource>` plural paths:** `permissions`, `roles`, `branches`, `users`, `customers`, `employees`, `vehicles`, `routes`, `active-routes`, `containers`, `deliveries`, `barcodes`, `pickups`, `invoices`, `journals`, `income-statements`.
