...
export function SidebarBrand({ compact = false, className, priority = false }: SidebarBrandProps) {
  if (compact) {
    return (
      <Link href="/" aria-label="Dashboard" className={cn("flex items-center justify-center", className)}>
  ...
  return (
    <Link href="/" aria-label="Dashboard" className={cn("flex min-w-0 items-center gap-3", className)}>
